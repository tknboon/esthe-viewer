...
assert.match(app, /const isCorrectionQueue = Boolean\(REGION_GEOCODE_BOUNDS\)/);
...
assert.match(workflow, /build-location-correction-queue\.mjs osaka/);
assert.match(workflow, /region-data\/tokyo\/location_correction_queue\.csv/);
assert.match(workflow, /region-data\/osaka\/location_correction_queue\.csv/);
assert.match(html, /config\/location-candidate\.js/);
assert.match(app, /getSafeSourceCoordinateCandidate/);
assert.match(app, /if \(!listingUrl \|\| !REGION_GEOCODE_BOUNDS\) return null/);
